...
and the image will be cropped accordingly.`,
		RunE: runCrop(fs, processorFactory),
	}

	cmd.Flags().IntP("width", "w", 0, "Desired width of the image")
	cmd.Flags().IntP("height", "H", 0, "Desired height of the image")
	cmd.Flags().IntP("gravity", "g", int(image.GravityCentre), `Gravity to use when cropping the image. 
Available options:
  0 - Centre (default)
  1 - North
  2 - East
		...
		return RunOperation(appConfig, OperationConfig{
			...
			ProcessorFunc: func(ctx context.Context, task processing.FileTask, stats *utils.ImageProcessingStats) error {
				return HandleImageProcessing(ctx, task, stats, processorFactory, appConfig, nil,
					image.WithCrop(width, height, gravity))
			},
		})
	}
}
