...
func runInit(fs filesystem.FileSystem) func(cmd *cobra.Command, args []string) error {
...
func NewInitCmd(fs filesystem.FileSystem) *cobra.Command {
	cmd := &cobra.Command{
		Use:     "init",
...
without having to pass flags every time you run a command.`,
	...
	cmd.Flags().BoolP("force", "f", false, "Overwrite existing config.yaml file")
